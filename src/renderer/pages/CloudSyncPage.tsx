...
export function CloudSyncPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (cloudStatus) {
        ...
        // Determine which emus are synced
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      if (s) {
        setStatus(s);
        if (s.running) { await saveCloudEnabled(true); showSuccess('Syncthing started!'); setLoading(false); }
        else {
          let attempts = 0;
          pollRef.current = setInterval(async () => {
            ...
            try {
              ...
              setStatus(latest);
              if (latest.running) { clearInterval(pollRef.current!); pollRef.current = null; await saveCloudEnabled(true); showSuccess('Syncthing started!'); setLoading(false); }
              else if (attempts >= 15) { clearInterval(pollRef.current!); pollRef.current = null; showError('Syncthing took too long to start.'); setLoading(false); }
            } catch { if (attempts >= 15) { clearInterval(pollRef.current!); pollRef.current = null; showError('Could not reach Syncthing.'); setLoading(false); } }
          }, 1000);
        }
  ...
  const handleStop = async () => {
    setLoading(true);
    try { const s = await window.omni.cloud.stop(); setStatus(s); await saveCloudEnabled(false); showSuccess('Syncthing stopped.'); }
  ...
  const handleAddDevice = async () => {
    ...
    try {
      ...
      if (ok) { showSuccess('Device added!'); setRemoteDeviceId(''); setRemoteDeviceName(''); loadData(); }
  ...
  else if (!forceReady && (status?.remoteDevices?.length ?? 0) === 0) step = 'setup';
  else step = 'ready';
    ...
    <div>
      ...
      {running && (
        <div style={{
          ...
          <span style={{ width: 8, height: 8, borderRadius: '50%', background: cloudEnabled ? 'var(--success, #44ff44)' : '#ffc844' }} />
      ...
      {running && (
        <>
          ...
          {/* Next button — step 3 only */}
          {step === 'setup' && (
            <button
              className="btn btn-primary"
              onClick={() => { setForceReady(true); saveCloudEnabled(true); }}
          ...
          {step === 'ready' && (
            <>
              {/* Paired Devices */}
              <div className="card mb-4">
                <div className="card-header"><h3>Paired Devices</h3></div>
                <div style={{ padding: '0 12px 12px' }}>
                  {status?.remoteDevices && status.remoteDevices.filter(d => d.id).length > 0 ? (
                    ...
                    <p className="text-muted text-sm" style={{ padding: 8 }}>No devices paired yet.</p>
                  )}
                </div>
              </div>
